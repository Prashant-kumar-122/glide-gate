...
function stageBadgeClass(stage: string): string {
  ...
  if (stage === 'ESCALATED') return 'bg-red-100 text-red-700'
  if (stage === 'KYC' || stage === 'REVIEW') return 'bg-amber-100 text-amber-700'
...
export default function CaseCard({ caseData, onClick }: Props) {
  ...
  return (
    <button
      onClick={onClick}
      className="w-full text-left rounded-2xl border border-gray-200 bg-white p-5 hover:shadow-md transition-all group"
    >
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap gap-1.5 mb-1">
            {caseData.selected_products.map((p) => (
              <span
                key={p}
                className="inline-flex items-center text-xs font-medium bg-gray-100 text-gray-700 px-2 py-0.5 rounded-full"
              >
                {formatProductName(p)}
              </span>
            ))}
