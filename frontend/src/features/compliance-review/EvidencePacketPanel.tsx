    ...
    <div className="p-4 space-y-4">
      ...
      <Section icon={<ShieldAlert size={16} className="text-amber-600" />} title="KYC Risk Assessment">
        ...
        <div className="mt-2 flex items-center gap-2">
          ...
          <span
            className={`text-xs font-semibold px-2 py-0.5 rounded-full ${
              RISK_BAND_COLOR[packet.kyc_result.risk_band] ?? 'bg-gray-100 text-gray-600'
            }`}
          >
        ...
        {packet.kyc_result.escalation_reasons.length > 0 && (
          <div className="mt-2">
            <p className="text-xs font-medium text-gray-600 mb-1">Escalation Reasons:</p>
          ...
          {Object.entries(packet.client_summary).map(([k, v]) => (
            v != null ? (
              <div key={k} className="contents">
                ...
                <dd className="text-gray-900 font-medium truncate">{String(v)}</dd>
              </div>
            ) : null
          ))}
        </dl>
        ...
        <div className="mt-2 text-xs space-y-1">
          ...
          <div className="flex flex-wrap gap-1">
            {Object.entries(packet.document_summary.by_status).map(([status, count]) => (
              <span key={status} className="px-1.5 py-0.5 rounded bg-gray-100 text-gray-700">
                {status}: {count}
              </span>
            ))}
          </div>
      ...
      <Section icon={<Package size={16} className="text-purple-600" />} title="Case Details">
        <dl className="mt-2 grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
          ...
          <dd className="text-gray-900 font-medium">{packet.case_summary.current_stage}</dd>
          <dt className="text-gray-500">Status</dt>
          <dd className="text-gray-900 font-medium">{packet.case_summary.status}</dd>
...
function ScoreCard({ label, value }: { label: string; value: number | null }) {
  ...
  const color =
    ...
    : pct >= 70 ? 'bg-red-100 text-red-800'
