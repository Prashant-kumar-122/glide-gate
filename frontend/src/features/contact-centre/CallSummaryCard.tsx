    ...
    return (
      <div className="rounded-xl border border-gray-200 bg-white p-4">
        ...
        <div className="space-y-2">
          {[90, 75, 60].map((w) => (
            <div key={w} className={`h-2 animate-pulse rounded bg-gray-100`} style={{ width: `${w}%` }} />
          ))}
        </div>
      </div>
    )
  ...
  if (isError || !summary) {
    return (
      <div className="rounded-xl border border-dashed border-gray-200 bg-gray-50 p-4 text-center">
        <Bot className="mx-auto h-6 w-6 text-gray-300" />
    ...
    <div className="rounded-xl border border-gray-200 bg-white p-4">
      <div className="mb-3 flex items-start justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-gray-700">AI Call Summary</span>
        </div>
        {onRefresh && (
      ...
      <p className="text-xs leading-relaxed text-gray-600">{summary.summary}</p>
        ...
        <div className="mt-3">
          ...
          <ul className="space-y-1">
            {summary.recommended_actions.map((action, i) => (
              <li key={i} className="flex items-start gap-1.5 text-xs font-medium text-gray-700">
