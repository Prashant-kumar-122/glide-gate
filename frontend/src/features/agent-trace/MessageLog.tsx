            ...
            {reversed.map((msg) => (
              <li key={msg.id} className="px-4 py-2.5">
                <div className="flex items-center gap-1.5">
                  <span className="text-[10px] font-semibold text-gray-700">{msg.fromAgent}</span>
                  <ArrowRight className="h-2.5 w-2.5 shrink-0 text-gray-400" />
                  <span className="text-[10px] font-semibold text-gray-700">{msg.toAgent}</span>
                  <span
                    className={[
                      'ml-auto shrink-0 rounded px-1 py-0.5 text-[9px] font-bold',
                      STATUS_COLORS[msg.status] ?? 'text-gray-500 bg-gray-100',
                    ].join(' ')}
                  >
                    {msg.status}
                  </span>
                </div>
                <p className="mt-0.5 text-[10px] text-gray-500">{msg.taskType}</p>
                <p className="mt-0.5 text-[9px] text-gray-300">{fmtTime(msg.timestamp)}</p>
              </li>
            ))}
          </ul>
        )}
