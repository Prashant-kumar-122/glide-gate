...
### 3.4 Fix streaming LLM fallback for mid-stream failures

**Problem:** `backend/app/services/llm/llm_fallback_chain.py` lines 32–47: the `stream` method catches exceptions and tries the next provider, but if a provider has already yielded chunks, those chunks are already sent to the client. The fallback can't "un-yield" them, resulting in a corrupted stream (partial response from provider A + full response from provider B).

**File to edit:** `backend/app/services/llm/llm_fallback_chain.py`

**What to do:** The fallback should only apply to the provider *selection* phase, not after streaming has begun. Buffer the first chunk to confirm the provider is healthy before committing to it:

```python
async def stream(self, request: LLMRequest) -> AsyncIterator[LLMStreamChunk]:
    providers = llm_provider_factory.create_fallback_ordered()
    errors: list[str] = []
    for provider in providers:
        try:
            gen = provider.stream(request)
            first = await gen.__anext__()  # confirm provider can produce output
            yield first
            async for chunk in gen:
                yield chunk
            return
        except StopAsyncIteration:
            return  # empty but valid stream
        except Exception as exc:
            logger.warning(f"LLMFallbackChain.stream: provider={provider.provider_name} failed: {exc!r}")
            errors.append(f"{provider.provider_name}: {exc}")
    raise RuntimeError(f"All LLM providers failed during streaming. Errors: {'; '.join(errors)}")
```

---

### 3.5 Add pagination to documents and clients list endpoints

**Problem:** `list_documents` and client list endpoints fetch all rows without limit/offset. With hundreds of documents per case this will degrade over time.

**Files to edit:** `backend/app/api/routers/documents.py`, `backend/app/api/routers/clients.py`

**What to do:** Add optional `limit` and `offset` query parameters (defaulting to sensible values) to list endpoints:
